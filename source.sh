...
echo -e "$RED $STORAGE_ROOT/daemon_builder/temp_coin_builds/${coindir} already exists.... Skipping $COL_RESET"
fi
echo -e "$RED If there was an error in the build use the build error options on the installer menu $COL_RESET"
exit 0
fi
...
if [[ ! -e '$STORAGE_ROOT/daemon_builder/temp_coin_builds/${coindir}/src/leveldb/build_detect_platform' ]]; then
  echo "$YELLOW build_detect_platform not found skipping $COL_RESET"
else
sudo chmod 777 $STORAGE_ROOT/daemon_builder/temp_coin_builds/${coindir}/src/leveldb/build_detect_platform
fi
