...
if [[ ! -e '$STORAGE_ROOT/daemon_builder/temp_coin_builds/${lastcoin}/src/leveldb/build_detect_platform' ]]; then
  echo "$RED => build_detect_platform not found skipping $COL_RESET"
else
...
clear
echo -e "YELLOW Installation of$GREEN ${coind::-1} $YELLOW is$GREEN completed$YELLOW and$GREEN running. $COL_RESET"
